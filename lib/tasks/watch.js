...
function WatchRegistry(taker) {
  ...
  this.set('ng:docs/watch', function() {
    ...
    watch(paths.scripts, {cwd: paths.cwd}).on('change', function(file) {
    ...
    watch(paths.docsScripts, {cwd: paths.cwd}).on('change', function(file) {
      return vfs.src(getRelativeFilePath(file, paths.cwd), {cwd: paths.cwd, base: paths.cwd})
        .pipe(factory.scripts.src.bind(self)({tmp: path.join(paths.tmp, paths.cwd)}))
        .on('error', watchErrorHandler)
        .pipe(reload({stream: true}));
    ...
    watch(paths.styles.replace(/([,{/])(\*\.)/g, '$1**/*.'), {cwd: paths.cwd}, function(files) {
    ...
    watch(paths.templates, {cwd: paths.cwd}).on('change', function(file) {
      return vfs.src(getRelativeFilePath(file, paths.cwd), {cwd: paths.cwd, base: paths.cwd})
    ...
    watch(docsPaths.scripts, {cwd: docsPaths.cwd}).on('change', function(file) {
    ...
    // Always watch all styles files
    watch(docsPaths.styles.replace(/([,{/])(\*\.)/g, '$1**/*.'), {cwd: docsPaths.cwd}, function(files) {
      // But only process root styles files
    ...
    watch(docsPaths.views, {cwd: docsPaths.cwd}).on('change', function(file) {
      return vfs.src(getRelativeFilePath(file, docsPaths.cwd), {cwd: docsPaths.cwd, base: docsPaths.cwd})
        .pipe(factory.views.src.bind(self)())
        .on('error', watchErrorHandler)
        .pipe(reload({stream: true}));
